...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --job-name="batch_geneBody"
...
# Usage: sbatch queue_geneBody.sh <ERR_list>
...
for ERR in `cat $ERR_FILE`;
    do
    if [ ! -r ${OUT_DIR}/${ERR}.geneBody_coverage.txt ]; then
        sbatch geneBody.sh ${BED_FILE} ${BAM_DIR}/${ERR}.Aligned.sortedByCoord.out.bam ${OUT_DIR}
    else
        echo "${OUT_DIR}/${ERR}.geneBody_coverage.txt already exists..."
    fi;
done
